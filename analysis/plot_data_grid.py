...
saturations = [2,5,10]
detunings = [-.5, -1, -2]
mag_fields = [0.5,1.5,3]
molecule = BaF

omega = 2 * np.pi * (cts.c / molecule.wave_length)
Isat = cts.hbar * omega ** 3 * (2 * np.pi * molecule.line_width_in_MHz * 1e6) / (12 * np.pi * cts.c ** 2)

fig, axs= plt.subplots(len(mag_fields), len(detunings), sharex=True, sharey='row')


velocity_values = [1, 1.5, 2.5]  # in m/s

for j, mag_field in enumerate(mag_fields):
    for i, detuning in enumerate(detunings):
        axs[i][j].axvline(x=velocity_to_si(np.array([-detuning]), molecule)[0], color='purple', linestyle='-', linewidth=2, alpha=0.4,
